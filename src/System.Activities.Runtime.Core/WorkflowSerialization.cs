...
namespace System.Activities.Runtime.Core
{
    public class WorkflowSerialization
    {
        ...
        private static IEnumerable<Type> runtime_types()
        {
            return from t in typeof(Activity).GetTypeInfo().Assembly
                                                .GetTypes()

                   let dc = t.GetCustomAttribute<DataContractAttribute>()
                   where dc != null
                   select t;
        }

        private static IEnumerable<Type> ref_types()
        {
            ...
            return types.Where(t => t.GetCustomAttribute<DataContractAttribute>() != null);
        }
